...
from baza import insert_user , insert_client,get_admin,get_user
from keyboards.inline import start_kb,qayta,lang_keyb,last
from states import Zaqaz,AdminState,Lang
from aiogram.fsm.context import FSMContext
from keyboards.reply import phone_kb,are_you_sure
...
@router.message(Zaqaz.sure)
async def zaqaz_sure(msg:Message,state:FSMContext):
    if msg.text == 'Xa' or msg.text == 'Да' or msg.text == 'Yes':
        
        data = await state.get_data()
        insert_client(data['loyiha'],data['full_name'],data['phone_number'],now)
        for i in get_admin():
            text=(
                        f"👤 Client: @{msg.from_user.username}\n\n"
        ...
        await msg.answer(txt, reply_markup=(ReplyKeyboardRemove()),)
        await state.clear()
    elif msg.text=='Yoq' or msg.text == 'Нет' or msg.text == 'No':
        tg_id = msg.from_user.id
        txt = languages[users_lang[tg_id]]['sure_no']
        await msg.answer(txt,reply_markup=qayta(users_lang[tg_id],'qayta_kb_1','qayta_kb_2'))
...
async def qayta(data:CallbackQuery,state:FSMContext):
    ...
    if action[1]=='rest':
        ...
        await data.message.answer(txt,start_kb(users_lang[tg_id],'start_kb_1','start_kb_2','start_kb_3'))
